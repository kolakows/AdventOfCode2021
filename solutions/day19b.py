...
def match(candidate, beacons):
    for b in beacons:
        for c in candidate:
            ...
            if len(common) >= 12:
                return set(zero_relative_beacons) - set(beacons), candidate_origin
    return None, None
...
while sc_not_matched:
    for i in tqdm(sc_not_matched.copy()):
        for rotated_beacons in rotations(scanners[i]):
            not_matched, origin = match(rotated_beacons, beacons)
            if not_matched:
...
for i in range(len(origins)):
    for j in range(len(origins)):
        if i != j:
            max_distance = max(max_distance, manhattan_distance(origins[i], origins[j]))
